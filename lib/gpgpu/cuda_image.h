...
public:
    ...
    CUDA_DEVHOST void accumulate(const CudaVector<float,3>& rgb, size_t x, size_t y, float exist_mul);
    CUDA_DEVHOST void set_pixel(const CudaVector<float,3>& rgb, size_t x, size_t y);
